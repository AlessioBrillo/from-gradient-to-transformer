...
## Part I — Where I Stand (State Review, Re-Verified 2026-08-28)
...
- **Clean-clone proof GREEN 2026-08-27** — fresh clone → `uv sync` → `make reproduce-quick` → `make verify-claims` → `pytest` → `ci-check` all pass. Phase 6 gate unlocked.
...
- This phase's ledger will be **ADR-0024** finalized — **MP-74's Sessions 2–8 complete the twentieth continuum ledger.**

### The CI Floor and Toolchains

197 tracked tests, ruff, blocking mypy, and markdownlint green at last release; `verify-claims` at 0. Verified gaps (facts, not hopes):
...
| Blocker | Status | What Must Happen in Sessions 2–4 |
|---------|--------|----------------------------------|
| **GPU Access** — P=113 grokking flagship never run on GPU | Colab notebook hardened, **Session 1 launched on Colab 2026-08-24** | **Sessions 2–3**: Monitor GPU run, download checkpoints/manifest at 3k+ epochs, verify verdict |
| **Induction Heads at Scale** — 0/8 heads at 3k epochs standard scale | Fresh-batches run completed 2026-08-14 (0 heads) | **Session 2**: Launch 10k epochs ×3 seeds with checkpointing every 500; track Step 1/Step 2 independently |
| **Clean-Clone Proof** — Phase 6 gate | **GREEN 2026-08-27** — full transcript in `06_production_ai/proofs/reproducible-from-clean-clone.md` | **Session 7**: Re-verification only (already complete) |
...
## Part III — The Roadmap, Step by Step (Continuum Law, Twentieth Execution, Sessions 2–8)

### The Frozen Candidate Set (Chosen at Session 0, Never Improvised)

| # | Candidate | Opens Only If | Window | Kill-Date | Status |
|---|-----------|---------------|--------|-----------|--------|
| R1 | **GPU Grokking 3-Seed P=113** | Always (primary flagship) | Session 1–3 | 2026-08-28 | **RUNNING** (launched Session 1) |
| R2 | **Extended Induction 10k Epochs ×3 Seeds** | Rung 1 < 0.3 diag+1 at 3k (confirmed) | Session 2–3 | 2026-08-30 | **PENDING** (launch Session 2) |
| R3 | **Neuron Ablation on Dense Grokking** | GPU run completes (uses existing checkpoints) | Session 2–3 | 2026-08-30 | **PENDING** (launch Session 2 parallel) |
| R4 | **Clean-Clone Reproducibility Proof** | Always (Phase 6 gate) | Session 4 | 2026-08-27 | **GREEN** (completed 2026-08-27) |
...
| R8 | **Gate-Debt Re-verification** | All MP-30–MP-36 rows | Session 1, 7 | 2026-09-03 | **PENDING** (Session 1 done) |
...
### Session 4 (~2 h) — Paper v20 Decision + Gate-Debt Update

**Objective**: Make paper v20 decision based on new numbers; update gate-debt ledger.

#### Actions

1. **Paper v20 Decision (Row 7)**:
...
2. **Gate-Debt Update (Row 8)**:
   - Update `checklists/gate-debt.md` with any new transcript links
   - Verify MP-30–MP-36 rows still PENDING/LAUNCHED/CLOSED-with-reason

**Exit Criteria**: Paper decision dated; gate-debt updated; ADR-0024 Row 7 stamped.
...
**Objective**: Final public shelf verification; teaching artifact polish; gate-debt final re-verification.
...
4. **Row 8 Final (Gate-Debt Re-verification)**:
   - Re-verify all MP-30–MP-36 rows with transcripts
...
**Exit Criteria**: Rows 5, 6, 7, 8 dated; artifact shipped with transcript; `gate-debt.md` complete.
...
## Part IV — Deep-Dive Study and Research Topics (Sessions 2–8)
...
|----------|----------|---------|
| **This roadmap** (promoted from companion at Session 0) | `00_meta/77_micro-phase-77-execution-roadmap.md` | Session 2 |
...
| **Clean-Clone Reproducibility Proof** (full transcript) | `06_production_ai/proofs/reproducible-from-clean-clone.md` | Session 4 (re-check), Session 7 (re-verification) |
...
| **Research Row Pre-registration Note** | `06_production_ai/notes/` | Session 2 |
...
### Ex-1 · GPU Execution Drill (Sessions 1–2)
...
### Ex-4 · Paper v20 Decision (Session 4)

**Goal**: Decide whether paper v20 compiles or "v19 is the record" memo.

```bash
# If R1/R2/R3 produced new numbers:
# Edit portfolio/paper/main.tex with new prose from manifests
# make paper  # verify compile (graceful if no TeX)
# Else:
# Write dated memo: "v19 is the record" — no new numbers from MP-74
```
...
**Status**: Ready for Session 2 execution — candidate set frozen, conditions explicit, no improvisation. The GPU unblock is the act of finally measuring the primary flagship on its native hardware; whatever it returns, the measurement is the contribution.
